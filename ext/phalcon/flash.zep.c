	...
	if (Z_TYPE_P(message) == IS_ARRAY) {
		...
		if (Z_TYPE_P(message) == IS_ARRAY) {
			ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(message), _1$$3)
			{
				...
				ZVAL_COPY(&msg, _1$$3);
				ZEPHIR_CALL_METHOD(&preparedMsg, this_ptr, "prepareescapedmessage", &_3, 91, &msg);
				zephir_check_call_status();
				ZEPHIR_CALL_METHOD(&htmlMessage, this_ptr, "preparehtmlmessage", &_4, 92, &type, &preparedMsg);
				zephir_check_call_status();
				if (implicitFlush) {
		...
		ZEPHIR_CALL_METHOD(&preparedMsg, this_ptr, "prepareescapedmessage", &_3, 91, message);
		zephir_check_call_status();
		ZEPHIR_CALL_METHOD(&htmlMessage, this_ptr, "preparehtmlmessage", &_4, 92, &type, &preparedMsg);
		zephir_check_call_status();
		if (implicitFlush) {
...
PHP_METHOD(Phalcon_Flash, prepareHtmlMessage) {
	...
	ZEPHIR_CALL_METHOD(&_4, this_ptr, "gettemplate", NULL, 93, &cssClasses);
	zephir_check_call_status();
	ZEPHIR_INIT_NVAR(&_2);
