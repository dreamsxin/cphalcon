...
PHP_METHOD(Phalcon_Events_Manager, attach) {
	...
	if (!(zephir_array_isset_fetch(&priorityQueue, &_0, &eventType, 0 TSRMLS_CC))) {
		...
		ZVAL_LONG(&_1$$4, 1);
		ZEPHIR_CALL_METHOD(NULL, &priorityQueue, "setextractflags", NULL, 214, &_1$$4);
		zephir_check_call_status();
	...
	ZEPHIR_CALL_METHOD(NULL, &priorityQueue, "insert", NULL, 215, handler, &_3);
	zephir_check_call_status();
	ZEPHIR_MM_RESTORE();
...
PHP_METHOD(Phalcon_Events_Manager, detach) {
	...
	if (zephir_array_isset_fetch(&priorityQueue, &_0, &eventType, 0 TSRMLS_CC)) {
		...
		ZVAL_LONG(&_1$$4, 1);
		ZEPHIR_CALL_METHOD(NULL, &newPriorityQueue, "setextractflags", NULL, 214, &_1$$4);
		zephir_check_call_status();
		ZVAL_LONG(&_1$$4, 3);
		...
		while (1) {
			...
			if (!ZEPHIR_IS_IDENTICAL(&_5$$5, handler)) {
				...
				ZEPHIR_CALL_METHOD(NULL, &newPriorityQueue, "insert", &_8, 215, &_6$$6, &_7$$6);
				zephir_check_call_status();
			}
		}
...
PHP_METHOD(Phalcon_Events_Manager, fire) {
	...
	ZEPHIR_CALL_METHOD(NULL, &event, "__construct", NULL, 216, &eventName, source, data, &_3);
	zephir_check_call_status();
	ZEPHIR_OBS_VAR(&fireEvents);
	if (zephir_array_isset_fetch(&fireEvents, &events, &type, 0 TSRMLS_CC)) {
		if (Z_TYPE_P(&fireEvents) == IS_OBJECT) {
			ZEPHIR_CALL_METHOD(&status, this_ptr, "firequeue", NULL, 217, &fireEvents, &event);
			zephir_check_call_status();
		}
	}
