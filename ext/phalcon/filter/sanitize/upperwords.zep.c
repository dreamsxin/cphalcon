...
PHP_METHOD(Phalcon_Filter_Sanitize_UpperWords, __invoke) {
	...
	if (1 == (zephir_function_exists_ex(SL("mb_convert_case") TSRMLS_CC) == SUCCESS)) {
		...
		ZEPHIR_RETURN_CALL_FUNCTION("mb_convert_case", NULL, 223, &input, &_0$$3, &_1$$3);
		zephir_check_call_status();
		RETURN_MM();
	}
	ZEPHIR_CALL_FUNCTION(&_2, "utf8_decode", NULL, 224, &input);
	zephir_check_call_status();
	ZEPHIR_RETURN_CALL_FUNCTION("ucwords", NULL, 226, &_2);
	zephir_check_call_status();
	RETURN_MM();
