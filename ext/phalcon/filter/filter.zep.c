...
PHP_METHOD(Phalcon_Filter_Filter, sanitize) {
	...
	if (Z_TYPE_P(sanitizers) == IS_ARRAY) {
		...
		if (Z_TYPE_P(sanitizers) == IS_ARRAY) {
			ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(sanitizers), _2$$3, _3$$3, _0$$3)
			{
				...
				if (_4$$5) {
					ZEPHIR_CALL_METHOD(&_5$$8, this_ptr, "processarrayvalues", &_6, 218, value, &sanitizerName, &sanitizerParams);
					zephir_check_call_status();
					ZEPHIR_CPY_WRT(value, &_5$$8);
				} else {
					ZEPHIR_CALL_METHOD(&_7$$9, this_ptr, "sanitizer", &_8, 219, value, &sanitizerName, &sanitizerParams);
					zephir_check_call_status();
			...
			while (1) {
				...
				zephir_check_call_status();
					...
					if (_9$$10) {
						ZEPHIR_CALL_METHOD(&_10$$13, this_ptr, "processarrayvalues", &_6, 218, value, &sanitizerName, &sanitizerParams);
						zephir_check_call_status();
						ZEPHIR_CPY_WRT(value, &_10$$13);
					} else {
						ZEPHIR_CALL_METHOD(&_11$$14, this_ptr, "sanitizer", &_8, 219, value, &sanitizerName, &sanitizerParams);
						zephir_check_call_status();
	...
	if (_12) {
		ZEPHIR_RETURN_CALL_METHOD(this_ptr, "processarrayvalues", &_6, 218, value, sanitizers);
		zephir_check_call_status();
		RETURN_MM();
	}
	ZEPHIR_RETURN_CALL_METHOD(this_ptr, "sanitizer", &_8, 219, value, sanitizers);
	zephir_check_call_status();
	RETURN_MM();
...
PHP_METHOD(Phalcon_Filter_Filter, processArrayValues) {
	...
	if (Z_TYPE_P(&values) == IS_ARRAY) {
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&values), _2, _3, _0)
		{
			...
			ZEPHIR_CALL_METHOD(&_4$$3, this_ptr, "sanitizer", &_5, 219, &itemValue, &sanitizerName, &sanitizerParams);
			zephir_check_call_status();
		...
		while (1) {
			...
			zephir_check_call_status();
				ZEPHIR_CALL_METHOD(&_6$$4, this_ptr, "sanitizer", &_5, 219, &itemValue, &sanitizerName, &sanitizerParams);
				zephir_check_call_status();
