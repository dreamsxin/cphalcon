...
PHP_METHOD(Phalcon_Db_Profiler, startProfile) {
	...
	ZEPHIR_CALL_METHOD(NULL, &activeProfile, "setsqlstatement", NULL, 181, &sqlStatement);
	zephir_check_call_status();
	if (Z_TYPE_P(sqlVariables) == IS_ARRAY) {
		ZEPHIR_CALL_METHOD(NULL, &activeProfile, "setsqlvariables", NULL, 182, sqlVariables);
		zephir_check_call_status();
	}
	if (Z_TYPE_P(sqlBindTypes) == IS_ARRAY) {
		ZEPHIR_CALL_METHOD(NULL, &activeProfile, "setsqlbindtypes", NULL, 183, sqlBindTypes);
		zephir_check_call_status();
	}
	ZEPHIR_INIT_VAR(&_0);
	zephir_microtime(&_0, &__$true TSRMLS_CC);
	ZEPHIR_CALL_METHOD(NULL, &activeProfile, "setinitialtime", NULL, 184, &_0);
	zephir_check_call_status();
