...
PHP_METHOD(Phalcon_Config_ConfigFactory, load) {
	...
	zval *config = NULL, config_sub, adapter, className, mode, callbacks, filePath, extension, oldConfig, _6, _7, _8, _15, _0$$3, _1$$3, _2$$3, _3$$3, _5$$5, _9$$9, _10$$9, _11$$11, _13$$13;
	...
	if (Z_TYPE_P(config) == IS_STRING) {
		...
		ZVAL_STRING(&_0$$3, ".");
		ZEPHIR_CALL_FUNCTION(&_1$$3, "strrchr", NULL, 158, config, &_0$$3);
		...
		if (UNEXPECTED(ZEPHIR_IS_EMPTY(&extension))) {
			ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_config_exception_ce, "You need to provide extension in file path", "phalcon/Config/ConfigFactory.zep", 47);
			return;
		}
	...
	if (UNEXPECTED(Z_TYPE_P(config) != IS_ARRAY)) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_config_exception_ce, "Config must be array or Phalcon\\Config object", "phalcon/Config/ConfigFactory.zep", 63);
		return;
	}
	ZEPHIR_OBS_VAR(&filePath);
	if (UNEXPECTED(!(zephir_array_isset_string_fetch(&filePath, config, SL("filePath"), 0)))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_config_exception_ce, "You must provide 'filePath' option in factory config parameter.", "phalcon/Config/ConfigFactory.zep", 69);
		return;
	}
	ZEPHIR_OBS_VAR(&adapter);
	if (UNEXPECTED(!(zephir_array_isset_string_fetch(&adapter, config, SL("adapter"), 0)))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_config_exception_ce, "You must provide 'adapter' option in factory config parameter.", "phalcon/Config/ConfigFactory.zep", 75);
		return;
	}
	ZEPHIR_INIT_VAR(&_6);
	zephir_camelize(&_6, &adapter, NULL  );
	ZEPHIR_INIT_VAR(&className);
	ZEPHIR_CONCAT_SV(&className, "Phalcon\\Config\\Adapter\\", &_6);
	ZEPHIR_INIT_VAR(&_7);
	ZVAL_STRING(&_7, ".");
	ZEPHIR_INIT_VAR(&_8);
	zephir_fast_strpos(&_8, &filePath, &_7, 0 );
	if (!(zephir_is_true(&_8))) {
		ZEPHIR_CALL_FUNCTION(&_9$$9, "lcfirst", NULL, 84, &adapter);
		zephir_check_call_status();
