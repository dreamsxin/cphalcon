...
PHP_METHOD(Phalcon_Config_Adapter_Ini, __construct) {
	...
	ZEPHIR_CALL_FUNCTION(&iniConfig, "parse_ini_file", NULL, 153, &filePath, &__$true, mode);
	...
	if (Z_TYPE_P(ini) == IS_ARRAY) {
		...
		if (Z_TYPE_P(ini) == IS_ARRAY) {
			ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(ini), _2$$3, _3$$3, _0$$3)
			{
				...
				ZVAL_COPY(&val, _0$$3);
				ZEPHIR_CALL_METHOD(&_4$$4, this_ptr, "cast", &_5, 154, &val);
				zephir_check_call_status();
			...
			while (1) {
				...
				zephir_check_call_status();
					ZEPHIR_CALL_METHOD(&_6$$5, this_ptr, "cast", &_5, 154, &val);
					zephir_check_call_status();
...
PHP_METHOD(Phalcon_Config_Adapter_Ini, parseIniString) {
	...
	zephir_create_array(return_value, 1, 0 TSRMLS_CC);
	ZEPHIR_CALL_METHOD(&_0, this_ptr, "parseinistring", NULL, 155, &path, value);
	zephir_check_call_status();
