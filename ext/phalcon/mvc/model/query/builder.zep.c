...
PHP_METHOD(Phalcon_Mvc_Model_Query_Builder, getModels) {
	...
	if (_1) {
		ZEPHIR_RETURN_CALL_CE_STATIC(phalcon_helper_arr_ce, "first", &_2, 245, &models);
		zephir_check_call_status();
		RETURN_MM();
	}
	RETURN_CCTOR(&models);
...
PHP_METHOD(Phalcon_Mvc_Model_Query_Builder, getPhql) {
	...
	if (zephir_is_numeric(&conditions)) {
		...
		if (zephir_fast_count_int(&primaryKeys TSRMLS_CC)) {
			...
			if (zephir_array_isset_long_fetch(&firstPrimaryKey, &primaryKeys, 0, 0 TSRMLS_CC)) {
				...
				ZEPHIR_CALL_METHOD(&_8$$13, this_ptr, "autoescape", NULL, 446, &model);
				zephir_check_call_status();
				ZEPHIR_CALL_METHOD(&_9$$13, this_ptr, "autoescape", NULL, 446, &attributeField);
				zephir_check_call_status();
	...
	if (Z_TYPE_P(&columns) != IS_NULL) {
			...
			if (Z_TYPE_P(&columns) == IS_ARRAY) {
				ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&columns), _13$$25, _14$$25, _11$$25)
				{
					...
					if (Z_TYPE_P(&columnAlias) == IS_LONG) {
						...
						ZEPHIR_CALL_METHOD(&_15$$28, this_ptr, "autoescape", NULL, 446, &columnAlias);
						zephir_check_call_status();
				...
				while (1) {
					...
					zephir_check_call_status();
						if (Z_TYPE_P(&columnAlias) == IS_LONG) {
							...
							ZEPHIR_CALL_METHOD(&_17$$31, this_ptr, "autoescape", NULL, 446, &columnAlias);
							zephir_check_call_status();
		...
		if (Z_TYPE_P(&models) == IS_ARRAY) {
			...
			if (Z_TYPE_P(&models) == IS_ARRAY) {
				ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&models), _22$$34, _23$$34, _20$$34)
				{
					...
					if (Z_TYPE_P(&modelColumnAlias) == IS_LONG) {
						ZEPHIR_CALL_METHOD(&_24$$36, this_ptr, "autoescape", NULL, 446, &model);
						zephir_check_call_status();
						ZEPHIR_CONCAT_VS(&selectedColumn, &_24$$36, ".*");
					} else {
						ZEPHIR_CALL_METHOD(&_25$$37, this_ptr, "autoescape", NULL, 446, &modelColumnAlias);
						zephir_check_call_status();
				...
				while (1) {
					...
					zephir_check_call_status();
						ZEPHIR_INIT_NVAR(&selectedColumn);
						if (Z_TYPE_P(&modelColumnAlias) == IS_LONG) {
							ZEPHIR_CALL_METHOD(&_26$$39, this_ptr, "autoescape", NULL, 446, &model);
							zephir_check_call_status();
							ZEPHIR_CONCAT_VS(&selectedColumn, &_26$$39, ".*");
						} else {
							ZEPHIR_CALL_METHOD(&_27$$40, this_ptr, "autoescape", NULL, 446, &modelColumnAlias);
							zephir_check_call_status();
			...
			ZEPHIR_CALL_METHOD(&_29$$41, this_ptr, "autoescape", NULL, 446, &models);
			zephir_check_call_status();
	...
	if (Z_TYPE_P(&models) == IS_ARRAY) {
		...
		if (Z_TYPE_P(&models) == IS_ARRAY) {
			ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&models), _33$$42, _34$$42, _31$$42)
			{
				...
				if (Z_TYPE_P(&modelAlias) == IS_STRING) {
					ZEPHIR_CALL_METHOD(&_35$$44, this_ptr, "autoescape", NULL, 446, &model);
					zephir_check_call_status();
					ZEPHIR_CALL_METHOD(&_36$$44, this_ptr, "autoescape", NULL, 446, &modelAlias);
					zephir_check_call_status();
					ZEPHIR_INIT_NVAR(&selectedModel);
					ZEPHIR_CONCAT_VSV(&selectedModel, &_35$$44, " AS ", &_36$$44);
				} else {
					ZEPHIR_CALL_METHOD(&selectedModel, this_ptr, "autoescape", NULL, 446, &model);
					zephir_check_call_status();
				}
			...
			while (1) {
				...
				zephir_check_call_status();
					if (Z_TYPE_P(&modelAlias) == IS_STRING) {
						ZEPHIR_CALL_METHOD(&_37$$47, this_ptr, "autoescape", NULL, 446, &model);
						zephir_check_call_status();
						ZEPHIR_CALL_METHOD(&_38$$47, this_ptr, "autoescape", NULL, 446, &modelAlias);
						zephir_check_call_status();
						ZEPHIR_INIT_NVAR(&selectedModel);
		...
		ZEPHIR_CALL_METHOD(&_41$$49, this_ptr, "autoescape", NULL, 446, &models);
		zephir_check_call_status();
		...
		if (Z_TYPE_P(&joins) == IS_ARRAY) {
			ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&joins), _43$$50)
			{
				...
				if (zephir_is_true(&joinType)) {
					ZEPHIR_CALL_METHOD(&_45$$52, this_ptr, "autoescape", NULL, 446, &joinModel);
					zephir_check_call_status();
					ZEPHIR_INIT_LNVAR(_46$$52);
					ZEPHIR_CONCAT_SVSV(&_46$$52, " ", &joinType, " JOIN ", &_45$$52);
					zephir_concat_self(&phql, &_46$$52 TSRMLS_CC);
				} else {
					ZEPHIR_CALL_METHOD(&_47$$53, this_ptr, "autoescape", NULL, 446, &joinModel);
					zephir_check_call_status();
				...
				if (zephir_is_true(&joinAlias)) {
					ZEPHIR_CALL_METHOD(&_49$$54, this_ptr, "autoescape", NULL, 446, &joinAlias);
					zephir_check_call_status();
			...
			while (1) {
				...
				zephir_check_call_status();
					...
					zephir_array_fetch_long(&joinType, &join, 3, PH_NOISY, "phalcon/Mvc/Model/Query/Builder.zep", 798 TSRMLS_CC);
					if (zephir_is_true(&joinType)) {
						ZEPHIR_CALL_METHOD(&_52$$57, this_ptr, "autoescape", NULL, 446, &joinModel);
						zephir_check_call_status();
						ZEPHIR_INIT_LNVAR(_53$$57);
						ZEPHIR_CONCAT_SVSV(&_53$$57, " ", &joinType, " JOIN ", &_52$$57);
						zephir_concat_self(&phql, &_53$$57 TSRMLS_CC);
					} else {
						ZEPHIR_CALL_METHOD(&_54$$58, this_ptr, "autoescape", NULL, 446, &joinModel);
						zephir_check_call_status();
					...
					if (zephir_is_true(&joinAlias)) {
						ZEPHIR_CALL_METHOD(&_56$$59, this_ptr, "autoescape", NULL, 446, &joinAlias);
						zephir_check_call_status();
	...
	if (Z_TYPE_P(&group) != IS_NULL) {
		...
		if (Z_TYPE_P(&group) == IS_ARRAY) {
			ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&group), _60$$63)
			{
				...
				ZEPHIR_CALL_METHOD(&_62$$64, this_ptr, "autoescape", NULL, 446, &groupItem);
				zephir_check_call_status();
			...
			while (1) {
				...
				zephir_check_call_status();
					ZEPHIR_CALL_METHOD(&_63$$65, this_ptr, "autoescape", NULL, 446, &groupItem);
					zephir_check_call_status();
			...
			if (Z_TYPE_P(&order) == IS_ARRAY) {
				ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&order), _67$$69)
				{
					...
					if (zephir_memnstr_str(&orderItem, SL(" "), "phalcon/Mvc/Model/Query/Builder.zep", 875) != 0) {
						...
						ZEPHIR_CALL_METHOD(&_69$$72, this_ptr, "autoescape", NULL, 446, &_70$$72);
						zephir_check_call_status();
					...
					ZEPHIR_CALL_METHOD(&_73$$70, this_ptr, "autoescape", NULL, 446, &orderItem);
					zephir_check_call_status();
				...
				while (1) {
					...
					zephir_check_call_status();
						...
						if (zephir_memnstr_str(&orderItem, SL(" "), "phalcon/Mvc/Model/Query/Builder.zep", 875) != 0) {
							...
							ZEPHIR_CALL_METHOD(&_74$$75, this_ptr, "autoescape", NULL, 446, &_75$$75);
							zephir_check_call_status();
						...
						ZEPHIR_CALL_METHOD(&_78$$73, this_ptr, "autoescape", NULL, 446, &orderItem);
						zephir_check_call_status();
	...
	if (Z_TYPE_P(&limit) != IS_NULL) {
		...
		if (zephir_is_numeric(&number)) {
			...
			ZVAL_LONG(&_83$$85, 10);
			ZEPHIR_CALL_FUNCTION(&_84$$85, "intval", NULL, 41, &number, &_83$$85);
			zephir_check_call_status();
			...
			if (_88$$85) {
				...
				ZVAL_LONG(&_89$$86, 10);
				ZEPHIR_CALL_FUNCTION(&_90$$86, "intval", NULL, 41, &offset, &_89$$86);
				zephir_check_call_status();
...
PHP_METHOD(Phalcon_Mvc_Model_Query_Builder, getQuery) {
	...
	ZEPHIR_CALL_METHOD(&phql, this_ptr, "getphql", NULL, 447);
	zephir_check_call_status();
	zephir_read_property(&_0, this_ptr, SL("container"), PH_NOISY_CC | PH_READONLY);
	ZEPHIR_CPY_WRT(&container, &_0);
...
PHP_METHOD(Phalcon_Mvc_Model_Query_Builder, limit) {
	...
	if (zephir_is_numeric(offset)) {
		...
		ZEPHIR_CALL_FUNCTION(&_3$$4, "abs", NULL, 222, &_2$$4);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Mvc_Model_Query_Builder, conditionBetween) {
	...
	if (UNEXPECTED(_0)) {
		...
		ZEPHIR_CALL_FUNCTION(&_3$$3, "sprintf", NULL, 170, &_2$$3, &operator);
		zephir_check_call_status();
