...
PHP_METHOD(Phalcon_Translate_Adapter_Csv, __construct) {
	...
	ZVAL_LONG(&_2, 0);
	ZEPHIR_CALL_METHOD(NULL, this_ptr, "load", NULL, 509, &_1, &_2, &delimiter, &enclosure);
	zephir_check_call_status();
	ZEPHIR_MM_RESTORE();
...
PHP_METHOD(Phalcon_Translate_Adapter_Csv, load) {
	...
	ZEPHIR_INIT_VAR(&_0);
	ZVAL_STRING(&_0, "rb");
	ZEPHIR_CALL_FUNCTION(&fileHandler, "fopen", NULL, 77, &file, &_0);
	zephir_check_call_status();
	...
	while (1) {
		...
		ZEPHIR_CALL_FUNCTION(&data, "fgetcsv", &_4, 510, &fileHandler, &_3$$4, &delimiter, &enclosure);
		zephir_check_call_status();
