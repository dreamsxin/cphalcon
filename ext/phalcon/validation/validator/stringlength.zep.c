...
PHP_METHOD(Phalcon_Validation_Validator_StringLength, validate) {
	...
	if (zephir_is_true(&isSetMax)) {
		...
		if (ZEPHIR_GT(&length, &maximum)) {
			...
			ZEPHIR_CALL_FUNCTION(&_6$$8, "strtr", NULL, 82, &message, &replacePairs);
			zephir_check_call_status();
			ZEPHIR_INIT_NVAR(&_5$$8);
			ZVAL_STRING(&_5$$8, "TooLong");
			ZEPHIR_CALL_METHOD(NULL, &_4$$8, "__construct", NULL, 376, &_6$$8, field, &_5$$8, &code);
			zephir_check_call_status();
	...
	if (zephir_is_true(&isSetMin)) {
		...
		if (ZEPHIR_LT(&length, &minimum)) {
			...
			ZEPHIR_CALL_FUNCTION(&_12$$11, "strtr", NULL, 82, &message, &replacePairs);
			zephir_check_call_status();
			ZEPHIR_INIT_NVAR(&_10$$11);
			ZVAL_STRING(&_10$$11, "TooShort");
			ZEPHIR_CALL_METHOD(NULL, &_9$$11, "__construct", NULL, 376, &_12$$11, field, &_10$$11, &code);
			zephir_check_call_status();
