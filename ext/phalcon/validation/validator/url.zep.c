...
PHP_METHOD(Phalcon_Validation_Validator_Url, validate) {
	...
	if (zephir_array_isset_string_fetch(&options, &_0, SL("options"), 0)) {
		ZVAL_LONG(&_1$$3, 273);
		ZEPHIR_CALL_FUNCTION(&result, "filter_var", NULL, 221, &value, &_1$$3, &options);
		zephir_check_call_status();
	} else {
		ZVAL_LONG(&_2$$4, 273);
		ZEPHIR_CALL_FUNCTION(&result, "filter_var", NULL, 221, &value, &_2$$4);
		zephir_check_call_status();
	}
	if (!(zephir_is_true(&result))) {
		...
		ZEPHIR_CALL_FUNCTION(&_4$$5, "strtr", NULL, 82, &message, &replacePairs);
		zephir_check_call_status();
		ZEPHIR_INIT_VAR(&_5$$5);
		ZVAL_STRING(&_5$$5, "Url");
		ZEPHIR_CALL_METHOD(NULL, &_3$$5, "__construct", NULL, 376, &_4$$5, field, &_5$$5, &code);
		zephir_check_call_status();
