...
PHP_METHOD(Phalcon_Di_Service, __set_state) {
	...
	ZEPHIR_CALL_METHOD(NULL, return_value, "__construct", NULL, 85, &definition, &shared);
	...
	if (Z_TYPE_P(&definition) == IS_STRING) {
		...
		if (Z_TYPE_P(&definition) == IS_OBJECT) {
			...
			if (Z_TYPE_P(&definition) == IS_ARRAY) {
				...
				ZEPHIR_CALL_METHOD(&instance, &builder, "build", NULL, 205, container, &definition, parameters);
				zephir_check_call_status();
			} else {
				found = 0;
			}
