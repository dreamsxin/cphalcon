...
PHP_METHOD(Phalcon_Config, merge) {
	...
	do {
		if (ZEPHIR_IS_STRING(&_0, "array")) {
			...
			ZEPHIR_CALL_METHOD(NULL, &config, "__construct", NULL, 8, configParam);
			zephir_check_call_status();
			break;
		}
	...
	ZEPHIR_RETURN_CALL_METHOD(this_ptr, "internalmerge", NULL, 11, &config);
	zephir_check_call_status();
	RETURN_MM();
...
PHP_METHOD(Phalcon_Config, offsetSet) {
	...
	if (Z_TYPE_P(value) == IS_ARRAY) {
		...
		ZEPHIR_CALL_METHOD(NULL, &_1$$3, "__construct", NULL, 8, value);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Config, path) {
	...
	while (1) {
		...
		ZEPHIR_CALL_FUNCTION(&key, "array_shift", &_1, 12, &keys);
		ZEPHIR_UNREF(&keys);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Config, internalMerge) {
	...
	zephir_check_call_status();
	ZEPHIR_CALL_FUNCTION(&_0, "get_object_vars", NULL, 9, config);
	zephir_check_call_status();
		...
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&_0), _3, _4, _1)
		{
			...
			if (zephir_fetch_property_zval(&localObject, instance, &property, PH_SILENT_CC)) {
				...
				if (_6$$5) {
					...
					if (_7$$6) {
						ZEPHIR_CALL_METHOD(NULL, this_ptr, "internalmerge", &_8, 11, &value, &localObject);
						zephir_check_call_status();
						continue;
					}
				}
