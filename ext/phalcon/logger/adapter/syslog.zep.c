...
PHP_METHOD(Phalcon_Logger_Adapter_Syslog, close) {
	...
	ZEPHIR_RETURN_CALL_FUNCTION("closelog", NULL, 367);
...
PHP_METHOD(Phalcon_Logger_Adapter_Syslog, process) {
	...
	ZEPHIR_CALL_FUNCTION(&result, "openlog", NULL, 368, &name, &option, &facility);
	zephir_check_call_status();
	if (!zephir_is_true(&result)) {
		...
		ZEPHIR_CALL_FUNCTION(&_3$$4, "sprintf", NULL, 170, &_2$$4, &name, &facility);
		zephir_check_call_status();
		ZEPHIR_CALL_METHOD(NULL, &_1$$4, "__construct", NULL, 369, &_3$$4);
		zephir_check_call_status();
	...
	ZEPHIR_CALL_METHOD(&level, this_ptr, "logleveltosyslog", NULL, 370, &_4);
	zephir_check_call_status();
	zephir_array_fetch_long(&_5, &message, 1, PH_NOISY | PH_READONLY, "phalcon/Logger/Adapter/Syslog.zep", 138 TSRMLS_CC);
	ZEPHIR_CALL_FUNCTION(NULL, "syslog", NULL, 371, &level, &_5);
	zephir_check_call_status();
	ZEPHIR_MM_RESTORE();
