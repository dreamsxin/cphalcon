...
PHP_METHOD(Phalcon_Logger_Logger, log) {
	...
	ZEPHIR_CALL_METHOD(&intLevel, this_ptr, "getlevelnumber", NULL, 373, level);
	zephir_check_call_status();
	zephir_get_strval(&_0, message);
...
PHP_METHOD(Phalcon_Logger_Logger, addMessage) {
	...
	zephir_time(&_1);
	ZVAL_LONG(&_0, level);
	ZEPHIR_CALL_METHOD(NULL, &item, "__construct", NULL, 374, &message, &levelName, &_0, &_1, &context);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_Logger_Logger, getLevelNumber) {
	...
	if (Z_TYPE_P(level) == IS_STRING) {
		...
		zephir_check_call_status();
		ZEPHIR_CALL_FUNCTION(&levels, "array_flip", NULL, 151, &_0$$3);
		zephir_check_call_status();
