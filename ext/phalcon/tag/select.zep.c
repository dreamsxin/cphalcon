...
PHP_METHOD(Phalcon_Tag_Select, selectField) {
	...
	if (Z_TYPE_P(&options) == IS_OBJECT) {
		...
		ZEPHIR_CALL_SELF(&_8$$23, "optionsfromresultset", &_9, 497, &options, &using, &value, &_11$$23);
		zephir_check_call_status();
		...
		if (Z_TYPE_P(&options) == IS_ARRAY) {
			...
			ZEPHIR_CALL_SELF(&_12$$25, "optionsfromarray", &_13, 498, &options, &value, &_15$$25);
			zephir_check_call_status();
...
PHP_METHOD(Phalcon_Tag_Select, optionsFromArray) {
	...
	if (Z_TYPE_P(&data) == IS_ARRAY) {
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&data), _2, _3, _0)
		{
			...
			ZEPHIR_CALL_FUNCTION(&escaped, "htmlspecialchars", &_4, 210, &optionValue);
			zephir_check_call_status();
			if (Z_TYPE_P(&optionText) == IS_ARRAY) {
				ZEPHIR_INIT_NVAR(&_5$$4);
				ZEPHIR_GET_CONSTANT(&_5$$4, "PHP_EOL");
				ZEPHIR_CALL_SELF(&_6$$4, "optionsfromarray", &_7, 498, &optionText, value, &closeOption);
				zephir_check_call_status();
		...
		while (1) {
			...
			zephir_check_call_status();
				...
				if (Z_TYPE_P(&optionText) == IS_ARRAY) {
					...
					ZEPHIR_CALL_SELF(&_17$$12, "optionsfromarray", &_7, 498, &optionText, value, &closeOption);
					zephir_check_call_status();
