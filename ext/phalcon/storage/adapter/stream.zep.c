...
PHP_METHOD(Phalcon_Storage_Adapter_Stream, __construct) {
	...
	ZEPHIR_CALL_CE_STATIC(&_3, phalcon_helper_str_ce, "dirseparator", &_4, 108, &cacheDir);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_Storage_Adapter_Stream, clear) {
	...
	ZEPHIR_CALL_CE_STATIC(&directory, phalcon_helper_str_ce, "dirseparator", &_0, 108, &_1);
	zephir_check_call_status();
	ZEPHIR_INIT_VAR(&_2);
	ZEPHIR_CONCAT_VS(&_2, &directory, "/*");
	ZEPHIR_CALL_METHOD(&iterator, this_ptr, "rglob", NULL, 109, &_2);
	zephir_check_call_status();
	...
	if (Z_TYPE_P(&iterator) == IS_ARRAY) {
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&iterator), _3)
		{
			...
			ZVAL_COPY(&file, _3);
			ZEPHIR_CALL_FUNCTION(&_5$$3, "is_dir", &_6, 110, &file);
			zephir_check_call_status();
			_7$$3 = !zephir_is_true(&_5$$3);
			if (_7$$3) {
				ZEPHIR_CALL_FUNCTION(&_8$$3, "unlink", &_9, 111, &file);
				zephir_check_call_status();
		...
		while (1) {
			...
			zephir_check_call_status();
				ZEPHIR_CALL_FUNCTION(&_10$$5, "is_dir", &_6, 110, &file);
				zephir_check_call_status();
				_11$$5 = !zephir_is_true(&_10$$5);
				if (_11$$5) {
					ZEPHIR_CALL_FUNCTION(&_12$$5, "unlink", &_9, 111, &file);
					zephir_check_call_status();
...
PHP_METHOD(Phalcon_Storage_Adapter_Stream, delete) {
	...
	ZEPHIR_CALL_METHOD(&directory, this_ptr, "getdir", NULL, 112, &key);
	zephir_check_call_status();
	ZEPHIR_INIT_VAR(&_1);
	ZEPHIR_CONCAT_VV(&_1, &directory, &key);
	ZEPHIR_RETURN_CALL_FUNCTION("unlink", NULL, 111, &_1);
	zephir_check_call_status();
	RETURN_MM();
...
PHP_METHOD(Phalcon_Storage_Adapter_Stream, get) {
	...
	ZEPHIR_CALL_FUNCTION(&_3, "json_last_error", NULL, 113);
	zephir_check_call_status();
	if (!ZEPHIR_IS_LONG_IDENTICAL(&_3, 0)) {
		RETVAL_ZVAL(defaultValue, 1, 0);
		RETURN_MM();
	}
	ZEPHIR_CALL_METHOD(&_4, this_ptr, "isexpired", NULL, 114, &payload);
	zephir_check_call_status();
	...
	if (Z_TYPE_P(&iterator) == IS_ARRAY) {
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&iterator), _3)
		{
			...
			if (!(zephir_is_true(&_5$$3))) {
				...
				ZEPHIR_CALL_CE_STATIC(&_8$$4, phalcon_helper_arr_ce, "last", &_9, 115, &split);
				zephir_check_call_status();
		...
		while (1) {
			...
			ZEPHIR_CALL_METHOD(&file, &iterator, "current", NULL, 0);
			zephir_check_call_status();
				ZEPHIR_CALL_FUNCTION(&_11$$5, "is_dir", &_6, 110, &file);
				zephir_check_call_status();
				if (!(zephir_is_true(&_11$$5))) {
					ZEPHIR_INIT_NVAR(&split);
					zephir_fast_explode_str(&split, SL("/"), &file, LONG_MAX TSRMLS_CC);
					zephir_read_property(&_12$$6, this_ptr, SL("prefix"), PH_NOISY_CC | PH_READONLY);
					ZEPHIR_CALL_CE_STATIC(&_13$$6, phalcon_helper_arr_ce, "last", &_9, 115, &split);
					zephir_check_call_status();
...
PHP_METHOD(Phalcon_Storage_Adapter_Stream, has) {
	...
	ZEPHIR_CPY_WRT(&payload, &_2);
	ZEPHIR_CALL_METHOD(&_3, this_ptr, "isexpired", NULL, 114, &payload);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_Storage_Adapter_Stream, set) {
	...
	ZEPHIR_CALL_METHOD(&directory, this_ptr, "getdir", NULL, 112, &key);
	zephir_check_call_status();
	ZEPHIR_CALL_FUNCTION(&_1, "is_dir", NULL, 110, &directory);
	zephir_check_call_status();
	if (!(zephir_is_true(&_1))) {
		ZVAL_LONG(&_2$$3, 0777);
		ZEPHIR_CALL_FUNCTION(NULL, "mkdir", NULL, 116, &directory, &_2$$3, &__$true);
		zephir_check_call_status();
	}
	ZEPHIR_INIT_VAR(&_3);
	ZEPHIR_INIT_VAR(&_4);
...
PHP_METHOD(Phalcon_Storage_Adapter_Stream, getDir) {
	...
	ZEPHIR_CALL_CE_STATIC(&dirFromFile, phalcon_helper_str_ce, "dirfromfile", &_2, 117, &key);
	zephir_check_call_status();
	ZEPHIR_CALL_CE_STATIC(&_3, phalcon_helper_str_ce, "dirseparator", &_4, 108, &dirPrefix);
	zephir_check_call_status();
	ZEPHIR_CALL_CE_STATIC(&_5, phalcon_helper_str_ce, "dirseparator", &_4, 108, &dirFromFile);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_Storage_Adapter_Stream, rglob) {
	...
	ZEPHIR_CALL_FUNCTION(&_0, "dirname", NULL, 118, &pattern);
	zephir_check_call_status();
	ZEPHIR_INIT_VAR(&dirName);
	ZEPHIR_CONCAT_VS(&dirName, &_0, "/*");
	ZEPHIR_CALL_FUNCTION(&files, "glob", NULL, 119, &pattern);
	zephir_check_call_status();
	ZEPHIR_INIT_VAR(&flags);
	ZVAL_LONG(&flags, (8192 | 4));
	ZEPHIR_CALL_FUNCTION(&_1, "glob", NULL, 119, &dirName, &flags);
	zephir_check_call_status();
	...
	if (Z_TYPE_P(&_1) == IS_ARRAY) {
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&_1), _2)
		{
			...
			ZEPHIR_CALL_METHOD(&recurse, this_ptr, "rglob", &_6, 109, &dir);
			zephir_check_call_status();
