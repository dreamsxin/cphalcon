...
PHP_METHOD(Phalcon_Annotations_Reader, parse) {
	...
	ZEPHIR_CALL_METHOD(NULL, &reflection, "__construct", NULL, 132, &className);
	zephir_check_call_status();
	ZEPHIR_CALL_METHOD(&comment, &reflection, "getdoccomment", NULL, 133);
	zephir_check_call_status();
	if (Z_TYPE_P(&comment) == IS_STRING) {
		ZEPHIR_INIT_VAR(&classAnnotations);
		ZEPHIR_CALL_METHOD(&_0$$3, &reflection, "getfilename", NULL, 134);
		zephir_check_call_status();
		ZEPHIR_CALL_METHOD(&_1$$3, &reflection, "getstartline", NULL, 135);
		zephir_check_call_status();
	...
	ZEPHIR_CALL_METHOD(&properties, &reflection, "getproperties", NULL, 136);
	zephir_check_call_status();
		...
		if (Z_TYPE_P(&properties) == IS_ARRAY) {
			ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&properties), _2$$5)
			{
				...
				if (Z_TYPE_P(&comment) == IS_STRING) {
					...
					ZEPHIR_CALL_METHOD(&_4$$7, &reflection, "getfilename", NULL, 134);
					zephir_check_call_status();
					ZVAL_LONG(&_5$$7, line);
			...
			while (1) {
				...
				zephir_check_call_status();
					...
					if (Z_TYPE_P(&comment) == IS_STRING) {
						ZEPHIR_INIT_NVAR(&propertyAnnotations);
						ZEPHIR_CALL_METHOD(&_7$$10, &reflection, "getfilename", NULL, 134);
						zephir_check_call_status();
	...
	ZEPHIR_CALL_METHOD(&methods, &reflection, "getmethods", NULL, 137);
	zephir_check_call_status();
