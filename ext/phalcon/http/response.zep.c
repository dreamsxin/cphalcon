...
PHP_METHOD(Phalcon_Http_Response, redirect) {
	...
	if (externalRedirect) {
		...
		if (_0$$5) {
			...
			ZVAL_STRING(&_1$$5, "://");
			ZEPHIR_CALL_FUNCTION(&_2$$5, "strstr", NULL, 256, location, &_1$$5);
			zephir_check_call_status();
...
PHP_METHOD(Phalcon_Http_Response, send) {
	...
	if (Z_TYPE_P(&content) != IS_NULL) {
		...
		if (_3$$5) {
			ZEPHIR_CALL_FUNCTION(NULL, "readfile", NULL, 347, &file);
			zephir_check_call_status();
		}
	}
	if (1) {
	...
	if (Z_TYPE_P(&currentHeadersRaw) == IS_ARRAY) {
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&currentHeadersRaw), _3, _4, _1)
		{
			...
			if (_5$$3) {
				...
				ZEPHIR_CALL_FUNCTION(&_7$$3, "strstr", &_8, 256, &key, &_6$$3);
				zephir_check_call_status();
		...
		while (1) {
			...
			zephir_check_call_status();
				...
				if (_10$$5) {
					...
					ZEPHIR_CALL_FUNCTION(&_12$$5, "strstr", &_8, 256, &key, &_11$$5);
					zephir_check_call_status();
