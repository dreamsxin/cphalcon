	...
	if (zephir_array_isset_string_fetch(&dataHeaders, &data, SL("headers"), 0)) {
		...
		if (Z_TYPE_P(&dataHeaders) == IS_ARRAY) {
			ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&dataHeaders), _2$$3, _3$$3, _0$$3)
			{
				...
				ZEPHIR_CALL_METHOD(NULL, &headers, "set", &_4, 349, &key, &value);
				zephir_check_call_status();
...
PHP_METHOD(Phalcon_Http_Response_Headers, send) {
	...
	ZEPHIR_CALL_FUNCTION(&_0, "headers_sent", NULL, 348);
	zephir_check_call_status();
	...
	if (Z_TYPE_P(&_1) == IS_ARRAY) {
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&_1), _4, _5, _2)
		{
			...
			if (Z_TYPE_P(&value) != IS_NULL) {
				...
				ZEPHIR_CALL_FUNCTION(NULL, "header", &_7, 350, &_6$$5, &__$true);
				zephir_check_call_status();
			} else {
				...
				if (_8$$6) {
					ZEPHIR_CALL_FUNCTION(NULL, "header", &_7, 350, &header, &__$true);
					zephir_check_call_status();
				} else {
					ZEPHIR_INIT_LNVAR(_12$$8);
					ZEPHIR_CONCAT_VS(&_12$$8, &header, ": ");
					ZEPHIR_CALL_FUNCTION(NULL, "header", &_7, 350, &_12$$8, &__$true);
					zephir_check_call_status();
				}
			}
		} ZEND_HASH_FOREACH_END();
		...
		while (1) {
			...
			zephir_check_call_status();
				if (Z_TYPE_P(&value) != IS_NULL) {
					...
					ZEPHIR_CALL_FUNCTION(NULL, "header", &_7, 350, &_13$$10, &__$true);
					zephir_check_call_status();
				} else {
					...
					if (_14$$11) {
						...
						ZEPHIR_CALL_FUNCTION(NULL, "header", &_7, 350, &_18$$13, &__$true);
						zephir_check_call_status();
					}
				}
