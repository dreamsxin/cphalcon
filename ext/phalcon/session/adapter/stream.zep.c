...
PHP_METHOD(Phalcon_Session_Adapter_Stream, __construct) {
	...
	if (!(zephir_array_isset_string_fetch(&path, &options, SL("savePath"), 0))) {
		...
		ZEPHIR_CALL_FUNCTION(&path, "ini_get", NULL, 481, &_2$$3);
		zephir_check_call_status();
	}
	ZEPHIR_CALL_FUNCTION(&_3, "is_writable", NULL, 316, &path);
	zephir_check_call_status();
	...
	ZEPHIR_CALL_CE_STATIC(&_6, phalcon_helper_str_ce, "dirseparator", &_7, 108, &path);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_Session_Adapter_Stream, destroy) {
	...
	if (_2) {
		ZEPHIR_CALL_FUNCTION(&_3, "is_file", NULL, 482, &file);
		zephir_check_call_status();
		_2 = zephir_is_true(&_3);
	}
	if (_2) {
		ZEPHIR_CALL_FUNCTION(NULL, "unlink", NULL, 111, &file);
		zephir_check_call_status();
	}
	RETURN_MM_BOOL(1);
...
PHP_METHOD(Phalcon_Session_Adapter_Stream, gc) {
	...
	ZEPHIR_CALL_FUNCTION(&_3, "glob", NULL, 119, &pattern);
	zephir_check_call_status();
	...
	if (Z_TYPE_P(&_3) == IS_ARRAY) {
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&_3), _4)
		{
			...
			if (_6$$3) {
				ZEPHIR_CALL_FUNCTION(&_7$$3, "is_file", &_8, 482, &file);
				zephir_check_call_status();
			...
			if (_9$$3) {
				ZEPHIR_CALL_FUNCTION(NULL, "unlink", &_11, 111, &file);
				zephir_check_call_status();
			}
		...
		while (1) {
			...
			zephir_check_call_status();
				...
				if (_12$$5) {
					ZEPHIR_CALL_FUNCTION(&_13$$5, "is_file", &_8, 482, &file);
					zephir_check_call_status();
